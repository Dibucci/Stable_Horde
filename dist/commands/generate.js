...
class default_1 extends command_1.Command {
    ...
    async run(ctx) {
        ...
        if (!ctx.client.config.generate?.enabled)
        ...
        if (!style?.prompt?.length)
            return ctx.error({ error: "Unable to find style for input" });
        if (ctx.client.config.generate?.require_login && !user_token)
            return ctx.error({ error: `You are required to ${await ctx.client.getSlashCommandTag("login")} to use ${await ctx.client.getSlashCommandTag("generate")}`, codeblock: false });
        if (ctx.client.config.generate?.blacklisted_words?.some(w => prompt.toLowerCase().includes(w.toLowerCase())))
            return ctx.error({ error: "Your prompt included one or more blacklisted words" });
        if (ctx.client.config.generate?.blacklisted_styles?.includes(style_raw.toLowerCase()))
            return ctx.error({ error: "The chosen style is blacklisted" });
        ...
        if (ctx.client.config.advanced?.dev)
            console.log(`${ctx.interaction.user.id} generated with prompt "${prompt}" (${generation_start?.id})`);
