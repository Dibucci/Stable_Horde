...
if (config.advanced_generate?.enabled) {
    command_data.addStringOption(new discord_js_1.SlashCommandStringOption()
        .setName("prompt")
        .setDescription("The prompt to generate an image with")
        .setRequired(true));
    if (config.advanced_generate?.user_restrictions?.allow_img2img) {
        command_data
            .addAttachmentOption(new discord_js_1.SlashCommandAttachmentOption()
            .setName("img2img")
    ...
    if (config.advanced_generate?.user_restrictions?.allow_sampler) {
        command_data
            ...
            .setChoices(...Object.keys(stable_horde_1.default.ModelGenerationInputStableSamplers).map(k => ({ name: k, value: k }))));
    }
...
class default_1 extends command_1.Command {
    ...
    async run(ctx) {
        if (!ctx.client.config.advanced_generate?.enabled)
            return ctx.error({ error: "Generation is disabled." });
        await ctx.interaction.deferReply({});
        let prompt = ctx.interaction.options.getString("prompt", true);
        const sampler = (ctx.interaction.options.getString("sampler") ?? ctx.client.config.advanced_generate?.default?.sampler ?? stable_horde_1.default.ModelGenerationInputStableSamplers.k_euler);
        ...
        if (ctx.client.config.advanced_generate?.require_login && !user_token)
            return ctx.error({ error: `You are required to ${await ctx.client.getSlashCommandTag("login")} to use ${await ctx.client.getSlashCommandTag("advanced_generate")}`, codeblock: false });
        if (ctx.client.config.advanced_generate?.blacklisted_words?.some(w => prompt.toLowerCase().includes(w.toLowerCase())))
            return ctx.error({ error: "Your prompt included one or more blacklisted words" });
        if (height % 64 !== 0)
            return ctx.error({ error: "Height must be a multiple of 64" });
        if (width % 64 !== 0)
            return ctx.error({ error: "Width must be a multiple of 64" });
        if (model && ctx.client.config.advanced_generate?.blacklisted_models?.includes(model))
            return ctx.error({ error: "This model is blacklisted" });
        if (model && model !== "YOLO" && !(await ctx.stable_horde_manager.getModels()).find(m => m.name === model))
            return ctx.error({ error: "Unable to find this model" });
        if (img && !can_bypass && !user_token)
            return ctx.error({ error: `You need to ${await ctx.client.getSlashCommandTag("login")} and agree to our ${await ctx.client.getSlashCommandTag("terms")} first before being able to use img2img`, codeblock: false });
        if (img && ctx.client.config.advanced_generate?.img2img?.require_stable_horde_account_oauth_connection && (!stable_horde_user || stable_horde_user.pseudonymous))
            return ctx.error({ error: "Your stable horde account needs to be created with a oauth connection" });
        if (img && !can_bypass && ctx.client.config.advanced_generate?.img2img?.require_nsfw_channel && (ctx.interaction.channel?.type !== discord_js_1.ChannelType.GuildText || !ctx.interaction.channel.nsfw))
            return ctx.error({ error: "This channel needs to be marked as age restricted to use img2img" });
        if (img && !img.contentType?.startsWith("image/"))
            return ctx.error({ error: "Image to Image input must be a image" });
        if (img && ((img.height ?? 0) > 3072 || (img.width ?? 0) > 3072))
            return ctx.error({ error: "Image to Image input too large (max. 3072 x 3072)" });
        if (img && !can_bypass && !ctx.client.config?.advanced_generate?.img2img?.allow_non_webp && img.contentType !== "image/webp")
            return ctx.error({ error: "You can only upload webp for img2img" });
        if (img && ctx.client.config.advanced_generate?.img2img?.whitelist?.only_allow_whitelist && !ctx.client.config.advanced_generate?.img2img?.whitelist?.user_ids?.includes(ctx.interaction.user.id))
            return ctx.error({ error: "You are not whitelisted to use img2img" });
        ...
        let img_data;
        if (img) {
            let img_data_res = await (0, centra_1.default)(img.url, "GET")
                .send();
            if (img.contentType === "image/webp")
                img_data = img_data_res.body;
            else {
                img_data = await buffer2webpbuffer(img_data_res.body, img.contentType?.replace("image/", ""), "-q 80").catch((e) => ctx.client.config.advanced?.dev ? console.error(e) : null);
                if (!img_data)
                    return ctx.error({
        ...
        if (ctx.client.config.advanced?.pre_check_prompts_for_suspicion?.enabled) {
            if (ctx.client.timeout_users.has(ctx.interaction.user.id))
                return ctx.error({ error: "Your previous prompt has been marked as suspicious.\nYou have been timed out, try again later." });
            ...
            if (ctx.client.timeout_users.has(ctx.interaction.user.id))
                return ctx.error({ error: "Your prompt has been marked as suspicious.\nTherefore you have been timed out!" });
        }
        const post_processing = [];
        if (gfpgan)
            post_processing.push(stable_horde_1.default.ModelGenerationInputPostProcessingTypes.GFPGAN);
        if (real_esrgan)
            post_processing.push(stable_horde_1.default.ModelGenerationInputPostProcessingTypes.RealESRGAN_x4plus);
        ...
        if (token === "0000000000" && ((generation_data.params?.width ?? 512) > 1024 || (generation_data.params?.height ?? 512) > 1024 || (generation_data.params?.steps ?? 512) > 100))
            return ctx.error({ error: "You need to be logged in to generate images with a size over 1024*1024 or more than 100 steps" });
        if (ctx.client.config.advanced?.dev) {
            console.log(token);
        ...
        if (!generation_start || !generation_start.id)
            return ctx.error({ error: `Unable to start generation: ${generation_start.message}` });
        if (ctx.client.config.logs?.enabled) {
            ...
            function logGeneration(type) {
                ...
                (0, fs_2.appendFileSync)(`${process.cwd()}${log_dir}/logs_${new Date().getMonth() + 1}-${new Date().getFullYear()}.${type}`, content);
            }
        }
        if (ctx.client.config.advanced?.dev)
            console.log(`${ctx.interaction.user.id} generated${!!img ? " img2img" : ""} with prompt "${prompt}" (${generation_start?.id})`);
...
ETA: <t:${Math.floor(Date.now() / 1000) + (start_status?.wait_time ?? 0)}:R>`
        ...
        const login_embed = new discord_js_1.EmbedBuilder({
        ...
        if (ctx.client.config.advanced?.dev)
            embed.setFooter({ text: generation_start.id });
        ...
        const inter = setInterval(async () => {
            ...
            if (status?.wait_time === 0 && prev_left !== 0)
                error_timeout = Date.now();
...
ETA: <t:${Math.floor(Date.now() / 1000) + (status?.wait_time ?? 0)}:R>`
            ...
            if (ctx.client.config.advanced?.dev)
                embed.setFooter({ text: generation_start?.id ?? "Unknown ID" });
        ...
        async function getCheckAndDisplayResult(precheck) {
            ...
            if (!status || status.faulted) {
                if (!done)
                    await message.edit({ content: "Image generation has been cancelled", embeds: [] });
                if (!precheck)
                    clearInterval(inter);
                return null;
            }
            ...
            if (!status.done)
                ...
                const image_map_r = images.generations?.map(async (g, i) => {
                    const req = await (0, centra_1.default)(g.img, "get").send();
                    if (ctx.client.config.advanced?.dev)
                        console.log(req);
                    const attachment = new discord_js_1.AttachmentBuilder(req.body, { name: `${g.seed ?? `image${i}`}.webp` });
                    const embed = new discord_js_1.EmbedBuilder({
                        title: `Image ${i + 1}`,
                ...
                if (ctx.client.config.advanced?.dev)
                    embeds.at(-1)?.setFooter({ text: `Generation ID ${generation_start.id}` });
                const files = image_map.map(i => i.attachment);
                if (img_data)
                    files.push(new discord_js_1.AttachmentBuilder(img_data, { name: "original.webp" }));
